...
const loadClient = async () => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api_clients.php?action=get_client&id=${clientId.value}${randomParam()}`
    )
...
const loadStats = async () => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api_clients.php?action=stats_client&id=${clientId.value}${randomParam()}`
    )
    const data = await response.json()
    if (data.success) {
...
const loadCredits = async () => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api_clients.php?action=list_credits&client_id=${clientId.value}${randomParam()}`
    )
    const data = await response.json()
    if (data.success) {
...
const loadPaiements = async () => {
  try {
    ...
    for (const credit of credits.value) {
      const response = await fetch(
        `${API_BASE_URL}/api_clients.php?action=historique_paiements&credit_id=${credit.id}${randomParam()}`
      )
      const data = await response.json()
      if (data.success) {
...
const saveCredit = async () => {
  ...
  saving.value = true
  try {
    formCredit.value.client_id = clientId.value
    
    const url = editingCredit.value 
      ? `${API_BASE_URL}/api_clients.php?action=update_credit`
      : `${API_BASE_URL}/api_clients.php?action=add_credit`
    
    const response = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(formCredit.value)
    })
...
const savePaiement = async () => {
  ...
  saving.value = true
  try {
    formPaiement.value.client_id = clientId.value
    
    const response = await fetch(`${API_BASE_URL}/api_clients.php?action=add_paiement`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(formPaiement.value)
    })
