...
try {
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'add_fournisseur') {
            ...
            // Construction stricte : id_entreprise et create_by toujours injectés côté backend
            $insertFields = ['id_entreprise', 'create_by', 'date_creation'];
            $params = [$id_entreprise, $user_id, date('Y-m-d H:i:s')];
            $map = [
            ...
            foreach ($map as $field) {
                if (isset($data[$field])) {
                    $insertFields[] = $field;
                    if (in_array($field, ['evaluation', 'note_evaluation'])) {
                        $params[] = floatval($data[$field]);
                    } elseif ($field === 'for_shop') {
            ...
            $sql = "INSERT INTO app_fournisseurs (" . implode(", ", $insertFields) . ") VALUES (" . rtrim(str_repeat('?,', count($insertFields)), ',') . ")";
            $db->query($sql, $params);
        ...
        if ($action === 'update_fournisseur') {
            ...
            // id_entreprise et modif_by injectés côté backend
            $allowedFields = [
                'categorie_fournisseur',
                'type_fournisseur',
                'nom',
            ...
            // Sécurisation : id_entreprise et modif_by toujours mis à jour côté backend
        ...
        // SUPPRESSION de la route par défaut dangereuse :
        // Si on arrive ici, c'est que l'action n'est pas reconnue, donc on bloque !
        echo json_encode(['success' => false, 'message' => 'Action POST non supportée ou non reconnue.']);
        exit;
    }
    ...
    if ($method === 'PUT') {
        ...
        // id_entreprise ne doit pas venir du frontend, mais du token
        $allowedFields = [
            'categorie_fournisseur',
            'type_fournisseur',
            'nom',
